...
import (
	...
	"github.com/cincout/mssfcore/errors"
...
func NewHttpBroker(addrs []string, opt ...broker.Option) broker.Broker {
	addr := ":0"
...
func (h *httpBroker) start() error {
	...
	logs.DefaultLog.Informational("Broker Listening on %s", l.Addr().String())
...
func (h *httpBroker) Publish(topic string, msg *broker.Message) error {
...
func (h *httpBroker) Subscribe(topic string, handler broker.Handler) (broker.Subscriber, error) {
...
func init() {
	broker.Register("http", NewHttpBroker)
}